...
const deleteCategories = async(req) => {
    ...
    // jika tidak ada, maka
    if (!result) throw new BadRequestError(`tidak ada kategori dengan id : ${id}`)

    // jika ada id nya, maka hapus dengan deleteOne
    await result.deleteOne();

    return result
}

const checkingCategories = async(categoryId) => {
    const result = await Categories.findById(categoryId);

    if (!result) throw new NotFoundError(`tidak ada categories dengan id : ${categoryId}`);
